...
import mypartitioner.{StatePartialKeyForOptimizedPartitioner, StatePartialKeyPartitioner}
...
object PartialKeyJoin {
  ...
  def main(args: Array[String]) {
    ...
    val sc = new SparkConf().setAppName("PartialKeyJoin_state_optimized")
    ...
    def mappingFuncPK(partitionId: Int, wordLtw: (String, BigInt), one: Option[Int],
                      state: State[mutable.Map[Int, Int]]):
    Option[((String, BigInt), mutable.Map[Int, Int])] = {
    ...
    val spec_rr = MyStateSpecWithIndex.function(mappingFuncPK _)
      .partitioner(new StatePartialKeyForOptimizedPartitioner(m, seeds))
