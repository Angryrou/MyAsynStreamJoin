...
object MyStateJoinUtils {
  ...
  def updataTimeTablesAndGetTriggers(partitionId: Int, port: Int, port_num: Int, port_trigger: BigInt): Option[BigInt] = {
    ...
    if (mp.size == port_num) {
      //      val old_global_trigger = mp.values.min
      val old_global_trigger = getPartitionTriggers(partitionId)
      mp(port) = port_trigger
      // 更新timeTables
