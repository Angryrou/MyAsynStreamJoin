...
import partitioner.RRPartitioner
...
object RRGrouping {
  ...
  def main(args: Array[String]) {
    if (args.length != 2) {
      System.err.println("Usage: RRGrouping_stateless <stream.json> duplicateRate")
      System.exit(1)
    }
    // 参数读取
    ...
    val sc = new SparkConf().setAppName("RRGrouping_stateless")
    ...
    val messages = KafkaUtils.createDirectStream[String, String, StringDecoder, StringDecoder](
      ...
      .transform(_.partitionBy(new RRPartitioner(m)))
