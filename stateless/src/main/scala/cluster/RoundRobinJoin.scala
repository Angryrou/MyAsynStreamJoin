...
/**
  ...
  *
  * Demo
  * kafka => L loaders =(round-robin)=> M mappers =(hashPartition)=> R reducers
  * L = 3, M = 10, R = 2
...
import mypartitioner.RoundRobinPartitioner
...
object RoundRobinJoin {
  def main(args: Array[String]) {
    if (args.length != 1) {
      System.err.println("Usage: RoundRobin <stream.json>")
      System.exit(1)
    }
    // 参数读取
...
//        Thread.sleep(1)
    ...
    val messages = KafkaUtils.createDirectStream[String, String, StringDecoder, StringDecoder](
      ...
      .transform(_.partitionBy(new RoundRobinPartitioner(m)))
